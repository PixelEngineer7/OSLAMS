<?php
    ...
    //Scope: Property Management Dashboard | List of All Registered Property On UDM OSALMS
session_start();
...
$result = mysqli_query($conn,"SELECT * FROM tblproperty ORDER BY propertyID ASC LIMIT $start_from, $limit");


$database=new DBHandler();
$p1=new property();
$PA=$p1->countPropertyM($database);
foreach ($PA as $rowA) 
  $PA=(int)$rowA['totalUnavailable'];

$p2=new property();
$PB=$p2->countPropertyA($database);
foreach ($PB as $rowB) 
  $PB=(int)$rowB['totalAvailable'];

$p3=new property();
$PC=$p3->countPropertyApproval($database);
foreach ($PC as $rowC) 
  $PC=(int)$rowC['total'];

$p4=new property();
$PD=$p4->countAll($database);
foreach ($PD as $rowD) 
  $PD=(int)$rowD['total'];
?>
<!DOCTYPE html>
<html lang="en">
<title>UDM OSLAMS Administrator | Property Management Dashboard </title>
<meta charset="UTF-8">
  ...
  <div class="row row-cols-1 row-cols-md-3 g-4">
    <div class="col">
      <div class="card shadow">
        <center><img src="../images/propertyApproved.png" class="img-responsive" alt="" style="padding-top:10px;"></center>
        <div class="card-body">
          <center><h5 class="card-title">Property Approval</h5></center>
          <center><p class="card-text">Approve Property on the UDM OSLAMS</p></center>
        </div>
        <div class="card-footer">
          <center><button type="cancel" class="btn btn-success" onclick="javascript:window.location='propertyApprove.php';">Approve Property</button></center>
        </div>
      </div>
    </div>
    <div class="col">
      <div class="card shadow">
       ...
       <div class="card-body">
         <center><h5 class="card-title">Modify Property Status </center>
          <center><p class="card-text">Modify Property Status in the UDM OSLAMS </p></center>
        </div>
        <div class="card-footer">
         <center><button type="cancel" class="btn btn-info" onclick="javascript:window.location='propertyStatus.php';">Modify Status</button></center>
       </div>
     </div>
   </div>
   <div class="col">
    <div class="card shadow">
      <center><img src="../images/rent.gif" class="img-responsive" alt="" style="padding-top:10px;"></center>
      <div class="card-body">
        <center> <h5 class="card-title">Property On Rent</h5></center>
        <center><p class="card-text">View Proposed Property on Rent by Landlords</p></center>
      </div>
      <div class="card-footer">
       <center><button type="cancel" class="btn btn-danger" onclick="javascript:window.location='../landlord/viewPropertyOnRent.php';">View Property</button></center>
     </div>
   </div>
 </div>
</div>
...
<div class="w3-row-padding w3-margin-bottom">
  <div class="w3-quarter">
    <div class="w3-container w3-indigo w3-padding-16 shadow">
      <div class="w3-left"><i class="fa fa-wrench fa-4x" aria-hidden="true"></i></div>
      <div class="w3-right">
        <h3 style="font-size:30px;"><?php echo $PA; ?></h3>
      </div>
      <div class="w3-clear"></div>
      <h6>Property Under Maintenance</h6>
    </div>
  </div>
  <div class="w3-quarter">
    <div class="w3-container w3-blue w3-padding-16 shadow">
      <div class="w3-left"><i class="fa fa-home fa-4x" aria-hidden="true"></i></i></div>
      <div class="w3-right">
        <h3 style="font-size:30px;"><?php echo $PB; ?></h3>
      </div>
      <div class="w3-clear"></div>
      <h4>Property Available For Rent</h4>
    </div>
  </div>
  <div class="w3-quarter">
    <div class="w3-container w3-teal w3-padding-16 shadow">
      <div class="w3-left"><i class="fa fa-check fa-4x" aria-hidden="true"></i></div>
      <div class="w3-right">
        <h3 style="font-size:30px;"><?php echo $PC; ?></h3>
      </div>
      <div class="w3-clear"></div>
      <h4>Property required Approval</h4>
    </div>
  </div>
  <div class="w3-quarter">
    <div class="w3-container w3-orange w3-text-white w3-padding-16 shadow">
      ...
      <div class="w3-right">
        <h2 style="font-size:30px;"><?php echo $PD; ?></h2>
      </div>
      <div class="w3-clear"></div>
      <h4>Total of Registered Property</h4>
    </div>
  </div>
</div>
      ...
      <table class="table table-hover" id="myTable">
        <thead >
          <tr>
            ...
            <th scope="col">Rental Fee Rs</th>
            <th scope="col">Deposit Fee Rs</th>
            <th scope="col">Property Status</th>
            <th scope="col">Area m2</th>
            <th onclick="sortTable(0)" scope="col">Address <i class="fa fa-sort"></i> </th>
            <th scope="col">Distance km</th>
            <th scope="col">WiFi Access</th>
            <th scope="col">Bathroom Access</th>
            <th scope="col">Kitchen Access</th>
          </tr>
        </thead>
        <?php while ($row = mysqli_fetch_array($result))   { ?>
          <tbody id="myTable">
            <tr>
              ...
              <td><?php echo $row['depositFee']; ?></td>
              <td><?php echo $row['status']; ?></td>
              <td><?php echo $row['area']; ?></td>
              <td><?php echo $row['address']; ?></td>
              <td><?php echo $row['distance']; ?></td>
              <td><?php  if ((int)$row['wifi']==1)
              echo "YES";
              else
                echo "NO";
            ?></td>
            <td><?php  if ((int)$row['bathroom']==1)
            echo "YES";
            else
              echo "NO";
          ?></td>
          <td><?php  if ((int)$row['kitchen']==1)
          echo "YES";
          else
            echo "NO";
        ?></td>
...
$result_db = mysqli_query($conn,"SELECT COUNT(propertyID) FROM tblproperty"); 
...
for ($i=1; $i<=$total_pages; $i++) {
  $pagLink .= "<li class='page-item'><a class='w3-button w3-hover-purple' href='propertyManagement.php?page=".$i."'>".$i."</a></li>"; 
}
echo $pagLink . "</ul>";  
?>
</div>

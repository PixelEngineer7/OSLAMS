<?php
    ...
    //Scope: Booking Management Dashboard | Rental Update by Landlord
session_start();
...
$result = mysqli_query($conn,"SELECT tblrental.rentalID , tblrental.tenantID , tblrental.propertyID , tblproperty.type , tblrental.startDate , tblrental.endDate, tblrental.status FROM tblrental INNER JOIN tblproperty ON tblrental.propertyID=tblproperty.propertyID ORDER BY rentalID ASC LIMIT $start_from, $limit");
...
<div class="w3-container">
  <div class="panel panel-info shadow">
    <div class="panel-heading"><h4><center>All Property Rental that have been made on UDM OSALMS</center></h4>
    </div>
     ...
     <table class="table table-hover" id="myTable">
      <thead>
        <tr>
          ...
          <th onclick="sortTable(0)" scope="col">Rental Status <i class="fa fa-sort"></i> </th>
      ...
      <?php while ($row = mysqli_fetch_array($result))   { ?>
        <tbody id="myTable">
          <tr>
            ...
            <td><?php echo "<a  class='btn btn-primary' href='rentalApprovalUpdate.php?action=update&QStrrentalID=".$row['rentalID']."'>Update</a>"; ?></td>
          </tr>
    ...
    $result_db = mysqli_query($conn,"SELECT COUNT(rentalID) FROM tblrental"); 
    ...
    for ($i=1; $i<=$total_pages; $i++) {
      $pagLink .= "<li class='page-item'><a class='w3-button w3-hover-pink' href='rentalApproval.php?page=".$i."'>".$i."</a></li>"; 
    }
    echo $pagLink . "</ul>";  
    ?>
  </div>

...
else if ($security[0]['category'] !='Administrator')
{
  header("location:../controller/logout.php");
    ...
    $getview=$viewRental->getAgreement($database);
    ...
    $result = mysqli_query($conn,"SELECT tblrental.rentalID , tblrental.tenantID , tblrental.propertyID , tblproperty.type , tblusers.name , tblusers.surname , tblrental.startDate , tblrental.endDate, tblrental.status FROM tblrental INNER JOIN tblproperty ON tblrental.propertyID=tblproperty.propertyID INNER JOIN tblusers ON tblrental.tenantID=tblusers.userID ORDER BY rentalID ASC LIMIT $start_from, $limit");
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
  ...
  <!-- START of Container for Welcome messages and Contents -->
   ...
   <div class="w3-container">
      <div class="panel panel-danger shadow">
        <div class="panel-heading"><h4><center>List of Rental Agreements </center></h4>
        </div>
      ...
      <thead>
        <tr>
          ...
          <th onclick="sortTable(1)" scope="col">Tenant Name <i class="fa fa-sort"></i> </th>
    ...
    <?php while ($row = mysqli_fetch_array($result))   { ?>
      <tbody id="myTable">
        <tr>
          ...
          <td><?php echo "<a  class='btn btn-danger' href='viewFullDetails.php?action=update&QStrtenantID=".$row['tenantID']."&QStrpropertyID=".$row['propertyID']."&QStrrentalID=".$row['rentalID']."'>View Agreement</a>"; ?></td>
        </tr>
 ...
 <?php  
          ...
          for ($i=1; $i<=$total_pages; $i++) {
                        $pagLink .= "<li class='page-item'><a class='w3-button w3-hover-cyan' href='viewAgreement.php?page=".$i."'>".$i."</a></li>"; 
          }
          echo $pagLink . "</ul>";  
?>
          </div>

<?php
    ...
    //Scope: Landlord Dashboard / Rental Update by Landlord
    session_start();
...
if (isset($_GET['QStrrentalID']))
  {
    ...
    $viewtenant=$viewrental->getUserRental($_SESSION['rentalID']);

    $userID=$viewtenant[0]['tenantID'];  
    ...
    $message = 'Hi User , The UDM Online Student Lodging Accomodation Management System (OSLAMS) Landlord has just updated (STATUS) one of your rental.Kindly login account to verify';
  ...
  if (isset($_POST['btnSubmit']))
  {
   ...
   $status=$_POST['status'];
   $rental = new rental();
   $rental->updateRental($_SESSION['rentalID'],$status);
...
<!DOCTYPE html>
<html>
<title>UDM OSLAMS  | Tenant Dashbord </title>
<meta charset="UTF-8">
...
<!-- START of Container to have Form for Tenant -->
  <div class="container">
    <div class="panel panel-info shadow">
       <div class="panel-heading"><h3><center>Update Rental Status</center></h3></div>
         <div class="panel-body">
            <?php foreach($viewtenant as $row) { ?>
           <form method="post" enctype="multipart/form-data">

             <div class="form-group">
               <label for="">Rental ID : </label>
                  <?php echo $row['rentalID']; ?>
            </div>
            ...
            <div class="form-group">
             ...
             <div class="form-group">
               <label for="">Agreement Read and Approved by Tenant : </label>
            ...
            <div class="form-group">
              <label>Rental Status</label>&nbsp;&nbsp;
                 <div class="form-check form-check-inline">
                    ...
                    <div class="form-check form-check-inline">
                      <input class="form-check-input" type="radio" name="status" id="inlineRadio2" value="Current" >
                      <label class="form-check-label" for="inlineRadio2">Current</label>
                   </div>  
                    <div class="form-check form-check-inline">
                      <input class="form-check-input" type="radio" name="status" id="inlineRadio2" value="Elapsed" >
                      <label class="form-check-label" for="inlineRadio2">Elapsed</label>
                   </div> 
                    <div class="form-check form-check-inline">
                      <input class="form-check-input" type="radio" name="status" id="inlineRadio2" value="Terminated" >
                      <label class="form-check-label" for="inlineRadio2">Terminated</label>
                   </div>      
            </div>

            <button type="submit" id="submit-button" class="btn btn-info" name="btnSubmit">Update Rental Status</button>&nbsp;
            <a href="manageRental.php" class="btn btn-danger">Cancel</a> 
